...
export const valueToObject: ValueToObject = ({ defaultValue, mappers, value }) => {
  ...
  const dv = defaultValue as Record<string, any>;
  ...
  switch (typeof value) {
    case 'boolean': {
      const mapper = (dv.$coerce as CoerceMap | undefined)?.boolean;
      if (mapper) {
    ...
    case 'function': {
      const mapper = (dv.$coerce as CoerceMap | undefined)?.function;
      if (mapper) {
    ...
    case 'number': {
      const mapper = (dv.$coerce as CoerceMap | undefined)?.number;
      if (mapper) {
        mergeResult(result, mapper(value));
    ...
    case 'string': {
      const mapper = (dv.$coerce as CoerceMap | undefined)?.string;
      if (mapper) {
        mergeResult(result, mapper(value));
