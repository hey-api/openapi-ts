 ...
 */
export const globalEvent = <ThrowOnError extends boolean = false>(options?: Options<never, ThrowOnError, GlobalEventResponse>) => (options?.client ?? client).sse.get<GlobalEventResponse, unknown, ThrowOnError>({ url: '/global/event', ...options });
...
export const eventSubscribe = <ThrowOnError extends boolean = false>(parameters?: {
    ...
    const params = buildClientParams([parameters], [{ args: [{ in: 'query', key: 'directory' }] }]);
    return (options?.client ?? client).sse.get<EventSubscribeResponse, unknown, ThrowOnError>({
        url: '/event',
        ...options,
        ...params
    });
