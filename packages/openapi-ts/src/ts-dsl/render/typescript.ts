...
export class TypeScriptRenderer implements Renderer {
  ...
  render(ctx: RenderContext<TsDsl>): string {
    ...
    for (const group of this.getImports(ctx)) {
      ...
      for (const imp of group) {
        imports += `${nodeToString(this.renderImport(astContext, imp))}\n`;
      }
    }
    ...
    for (const node of ctx.file.nodes) {
      if (nodes) nodes += '\n';
      nodes += `${nodeToString(node.toAst(astContext))}\n`;
    }
    ...
    for (const group of this.getExports(ctx)) {
      ...
      for (const exp of group) {
        exports += `${nodeToString(this.renderExport(astContext, exp))}\n`;
      }
    }
  ...
  private renderExport(
    ...
    return ts.factory.createExportDeclaration(
      ...
      group.isTypeOnly,
      exportClause,
      $.literal(group.modulePath).toAst(ctx),
    );
  }
