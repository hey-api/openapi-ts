...
describe('operationNameFn', () => {
  ...
  it.each([
    ...
    },
    {
      ...
      operationId: 'fooBar',
      options: options2,
      path: '/api/v{api-version}/users/{userId}/location/{locationId}',
    },
    {
      expected: 'getAllUsers',
      method: 'GET',
    ...
    ({ path, method, options, operationId, expected }) => {
      setConfig(options);
      expect(
        operationNameFn({ id: operationId, method, path } as Operation),
      ).toBe(expected);
    },
  );
});
