...
export const processIndex = async ({ files }: { files: Record<string, TypeScriptFile> }): Promise<void> => {
    ...
    if (config.exportCore) {
        ...
        if (config.serviceResponse === 'response') {
            files.index.add(compiler.export.named({ isTypeOnly: true, name: 'ApiResult' }, './core/ApiResult'));
        }
        if (config.name) {
            files.index.add(compiler.export.named('BaseHttpRequest', './core/BaseHttpRequest'));
        }
        if (config.client !== 'angular') {
            files.index.add(compiler.export.named(['CancelablePromise', 'CancelError'], './core/CancelablePromise'));
        }
        files.index.add(
