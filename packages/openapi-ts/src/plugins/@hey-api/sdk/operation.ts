...
import { createRequestKeyMapper } from './requestKeyMapper';
...
export const operationStatements = ({
  ...
  const requestKeyMapperInfo = createRequestKeyMapper({ operation, plugin });
  if (requestKeyMapperInfo) {
    // Add import for the shared mapper function
    f.addImport({
      from: requestKeyMapperInfo.file,
      names: [requestKeyMapperInfo.placeholder],
    });

    requestOptions.push({
      key: 'requestKeyMapper',
      value: requestKeyMapperInfo.placeholder,
    });
  }
    ...
    tsc.returnFunctionCall({
      args: [
        tsc.objectExpression({
          identifiers: ['requestKeyMapper', 'responseTransformer'],
          obj: requestOptions,
        }),
      ],
      name: functionName,
