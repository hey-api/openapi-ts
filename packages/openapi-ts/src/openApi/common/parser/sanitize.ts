...
export function sanitizeTypeName(name: string): string {
    return name.replace(/^[^$_\p{ID_Start}]+/u, '').replace(/[^$\u200c\u200d\p{ID_Continue}]/gu, '_');
}
...
export function sanitizeServiceName(name: string): string {
    return name.replace(/^[^\p{ID_Start}]+/u, '').replace(/[^$\u200c\u200d\p{ID_Continue}]/gu, '-');
}
...
export function sanitizeOperationParameterName(name: string): string {
