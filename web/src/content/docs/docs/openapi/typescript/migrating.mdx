...
This is an internal change that simplifies the configuration and plugin APIs. The generated output should be unaffected, please [open an issue](https://github.com/hey-api/openapi-ts/issues) if that's not the case. If you have custom plugins, refer to the [custom plugin guide](/docs/openapi/typescript/plugins/custom) for the latest instructions.
