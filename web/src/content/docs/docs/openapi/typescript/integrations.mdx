...
:::caution
This feature is in development! 🎉 Try it out and provide feedback on [GitHub](https://github.com/orgs/hey-api/discussions/1773).
:::
...
```yaml title="github-workflow.yml"
...
<Tabs>
  <TabItem label="Hey API">
    <Code code="npx @hey-api/openapi-ts -i hey-api/backend -o src/client" lang="sh" />
  </TabItem>
  <TabItem label="OpenAPI TypeScript">
    <Code
      code={`npx openapi-typescript \\\r
    https://get.heyapi.dev/hey-api/backend \\\r
    -o schema.ts`}
      lang="sh"
    />
  </TabItem>
  <TabItem label="Orval">
    <Code
      code={`npx orval \\\r
    --input https://get.heyapi.dev/hey-api/backend \\\r
