---
title: MSW v2 Plugin
description: Generate MSW v2 handlers from OpenAPI with the MSW plugin for openapi-ts. Fully compatible with validators, transformers, and all core features.
sidebar:
  label: MSW
custom:
  version: v2
...
<FeatureStatus issueNumber={1486} status="in-progress" />
...
In your [configuration](/docs/openapi/typescript/get-started), add `msw` to your plugins and you'll be ready to generate MSW artifacts. 🎉
...
By default, `source` is set to `['@hey-api/examples']`, which embeds OpenAPI examples in the generated output. To disable this, set `source` to an empty array.
...
You can view the complete list of options in the [UserConfig](https://github.com/hey-api/hey-api/blob/main/packages/openapi-ts/src/plugins/msw/types.ts) interface.
