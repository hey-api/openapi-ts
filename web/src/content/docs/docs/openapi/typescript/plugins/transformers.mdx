...
To use the [Temporal API](https://tc39.es/proposal-temporal/docs/) instead of `Date`, set `dates` to `'temporal'`. `Temporal` is imported from [`temporal-polyfill`](https://www.npmjs.com/package/temporal-polyfill), which must be installed in your project.
...
`date-time` is emitted as `Temporal.Instant` and `date` as `Temporal.PlainDate`. Response transformers call `Temporal.Instant.from(value)` and `Temporal.PlainDate.from(value)` respectively.
