...
Currently available for [TypeScript](/docs/openapi/typescript/plugins/typescript), [Valibot](/docs/openapi/typescript/plugins/valibot), and [Zod](/docs/openapi/typescript/plugins/zod).
...
We're declaring the `Big` import in `~imports` and accessing it from `plugin.imports`. This replaces the previous `plugin.symbolOnce()` approach.
