...
By default, all SDK methods accept an object with namespaced properties for each parameter type: `query`, `path`, `header`, and `body`.

<Tabs>
  <TabItem label="example">
```ts title="app.ts"
import { addPet } from './sdk.gen.ts';
...
If you prefer a more compact syntax, you can set `paramsStructure` to `flat`. This will merge all parameter types into a single object, which is especially useful for endpoints with only one parameter type.
...
:::note
The SDK plugin currently supports only the `bearer` and `basic` auth schemes. [Open an issue](https://github.com/hey-api/hey-api/issues) if you'd like support for additional mechanisms.
:::
...
Every SDK method accepts an optional `meta` object. You can use it to pass arbitrary values that aren't part of the SDK function and read them back in your [client](/docs/openapi/typescript/clients) interceptors or middleware.
...
By default `meta` is untyped, so typos and wrong-typed values compile silently. To make it type-safe, augment the `ClientMeta` interface exported by your client. After augmentation, `meta` is fully type-checked at the call site.

<Tabs>
  <TabItem label="types">
```ts title="overrides.d.ts"
...
await addPet({
  meta: {
    timeout: 5000, // ✅ correctly typed
    timeoutt: 5000, // ❌ unknown key
    retry: '5s', // ❌ wrong type
  },
});
```
...
You can view the complete list of options in the [UserConfig](https://github.com/hey-api/hey-api/blob/main/packages/openapi-ts/src/plugins/@hey-api/sdk/types.ts) interface.
