...
::: warning
Request interceptors registered through `client.interceptors.request` apply to SSE connections, including on each reconnect attempt. Native ofetch hooks (`onRequest`, `onResponse`) do not apply — SSE uses the Fetch API directly.
:::
